...
class CRNNDataset(Dataset):
    ...
    def get_label(img_path):
        file_name = os.path.splitext(os.path.split(img_path)[1])[0]  # Lấy tên file không bao gồm phần mở rộng
        label_parts = file_name.split('_')
        if len(label_parts) >= 2:
            label = label_parts[1]  # Trích xuất nhãn từ tên file
        else:
            label = ''  # Nếu không tìm thấy nhãn, gán nhãn là chuỗi rỗng
        return label
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_dir",default=r'E:\project_ai\Train\crnn_recognition\label\result_label_1' ,help="path to dataset")
    parser.add_argument("--batch_size", default=128)
    parser.add_argument("--alphabets", default='_#0123456789abcddefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ,.-%.', help="alphabets used in dataset")
